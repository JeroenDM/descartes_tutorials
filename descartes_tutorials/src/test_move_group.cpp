...
int main(int argc, char **argv)
{
  ...
  Eigen::Quaterniond ori(start_pose.linear());
  
  moveit::planning_interface::MoveGroup group("manipulator");
  geometry_msgs::Pose target_pose1;
  target_pose1.orientation.w = ori.w();
  target_pose1.orientation.x = ori.x();
  target_pose1.orientation.y = ori.y();
  target_pose1.orientation.z = ori.z();
  target_pose1.position.x = pos[0];
  target_pose1.position.y = pos[1];
  target_pose1.position.z = pos[2];
